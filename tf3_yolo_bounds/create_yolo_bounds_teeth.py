    ...
    def _run_case(split_name, case_id):
        for suffix in ["", "_mirrored"]:
                ...
                box = [max(int(crop_0[0] * largest_dim_orig) - margin, 0), 
                       ...
                       max(int(crop_2[0] * largest_dim_orig) - margin, 0), 
                       min(int(crop_2[1] * largest_dim_orig) + margin, largest_dim_orig), 
                       ]
                ...
                for init_label in [2, 3]:
                    ...
                    image_mip_in = im_square[:, :, z0_slice : z1_slice]
                    label_mip_in = lab_square[:, :, z0_slice : z1_slice]
                    
                    # Set pulp labels as teeth labels.
                    label_mip_in = np.where(label_mip_in > 100, label_mip_in - 100, label_mip_in)

                    path_image_mip_out = path_yolo_out / split_name / verbose_label / "images" / f"{case_id}{suffix}.png"
                    path_label_mip_out = path_yolo_out / split_name / verbose_label / "label_review" / f"{case_id}{suffix}.png"
                    path_yolo_txt_out = path_yolo_out / split_name / verbose_label / "labels" / f"{case_id}{suffix}.txt"
                    ...
                    for label in dict_labels:
                        bb = calculate_2d_bounds_as_fraction(label_mip_in, [label, label+100], 2)

                        if bb is None:
                            print(f"{path_ct_image.name} - No bounds available for {label}")
                        else:
