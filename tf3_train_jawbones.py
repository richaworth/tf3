...
def main(path_output_dir: Path = Path("C:/data/tf3_localiser_output/")):
    ...
    initial_transforms = [
        ...
        LabelFilterd("localiser", [1, 2, 3, 4, 10, 103, 104, 105]),  # Jaw bones are 1, 2. Implants are 10. Canals are 3, 4, 103, 104, 105.
        AsDiscreted("localiser", to_onehot=n_labels),  # Convert filtered labels to 1..N labels.
    ...
    for case_ids, ld in [(d_case_ids["train"], ld_train), (d_case_ids["val"], ld_val), (d_case_ids["test"], ld_test)]:
        for c in case_ids:
            d = {"image": path_images / f"{c}.nii.gz", 
                ...
                "sd": path_localiser_labels / f"{c}_cropped.nii.gz"}
            dm = {"image": path_images / f"{c}_mirrored.nii.gz", 
                "label": path_labels / f"{c}_mirrored.nii.gz", 
                "sd": path_localiser_labels / f"{c}_mirrored_cropped.nii.gz"}
            ld.append(d)
            ld.append(dm)
